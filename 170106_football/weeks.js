...
var Weeks1 = (function () {
    ...
    Weeks1.prototype.precalc = function () {
        var _this = this;
        ...
        this.weeks = d3.scaleBand()
            .domain(d3.range(this.dao.max_week + 1).map(function (x) { return '' + x; }))
            .paddingInner(0.1)
            .range([0, this.width]);
        this.cells = this.svg.selectAll('.cell')
            ...
            .enter().append('g')
            .classed('cell', true)
            .attr('transform', function (match) { return ("translate(" + _this.weeks(match['week']) + ", " + _this.teams(match['cur_team']) + ")"); });
    ...
    Weeks1.prototype.first_draw = function () {
        var _this = this;
        ...
        this.svg.selectAll('.team-hor')
            ...
            .text(R.identity)
            .attr('transform', function (team) { return ("translate(-10, " + _this.teams(team) + ") translate(0, " + _this.teams.bandwidth() * 0.7 + ") rotate(-45)"); })
            .attr('text-anchor', 'end');
        ...
        this.prob_g.selectAll('.team_titles')
            ...
            .attr('x', function (d) { return ({ cur: 0, other: _this.ind_width }[d]); })
            .attr('y', 10)
        ...
        this.stat_g.selectAll('text.labels')
            ...
            .attr('fill', 'white')
            .classed('labels', true);
        this.cells.on('mouseover', function (match) {
            _this.highlight_team(match['cur_team']);
            _this.highlight_match(match);
        }).on('mouseout', this.reset.bind(this));
        this.svg.selectAll('.team-hor').on('mouseover', this.highlight_team.bind(this))
            .on('mouseout', this.reset.bind(this));
    };
    Weeks1.prototype.reset = function () {
        this.svg.selectAll('.team-hor')
            .attr('fill', 'black')
        ...
        this.svg.selectAll('.cell rect')
            .attr('fill', function (match) { return common_1.Common1.res_interp[match['res']](match['prob'][match['res']]); });
    };
    Weeks1.prototype.highlight_team = function (team) {
        var _this = this;
        var sel = this.svg.selectAll('.cell')
        ...
        this.dao.matches1.forEach(function (match) { if (match.cur_team == team) {
            stats[match.res]++;
        } });
        ...
        this.svg.selectAll('.stat_g rect')
            ...
            .attr('height', 20)
            .attr('fill', function (letter) { return common_1.Common1.clr[letter]; });
        this.svg.selectAll('.stat_g text.labels')
            .attr('x', function (letter) { return _this.prob_scale((left[letter] + stats[letter] / 2) / total); })
            .attr('y', 30)
            .text(function (letter) { return stats[letter]; });
        this.svg.selectAll('.stat_g text.title')
            .text("\"" + team + "\" statistics");
    };
    Weeks1.prototype.highlight_match = function (match) {
        var _this = this;
        this.svg.selectAll('.prob_g')
            .attr('visibility', 'visible');
        ...
        this.svg.selectAll('.prob_g rect')
            .attr('x', function (letter) { return _this.prob_scale(left[letter] / total); })
            .attr('y', 20)
            .attr('width', function (letter) { return _this.prob_scale(probs[letter] / total); })
            .attr('height', 20)
            .attr('fill', function (letter) { return common_1.Common1.res_interp[letter](match.prob[letter]); });
        this.svg.selectAll('.prob_g text.labels')
            .attr('x', function (letter) { return _this.prob_scale((left[letter] + probs[letter] / 2) / total); })
            .attr('y', 30)
    ...
    Weeks1.prototype.launch = function () {
        this.precalc();
        this.first_draw();
        this.reset();
    };
