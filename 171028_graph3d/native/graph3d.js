...
function initGL(canvas) {
    try {
        var res = canvas.getContext('webgl');
        if (glt != null) {
            gl = glt;
...
function getShader(str, shader_type) {
    var shader;
...
function on_load() {
    ...
    oReq.addEventListener('load', on_json_load);
    oReq.open('GET', '../jupyter/metal_2.json');
    oReq.send();
}
